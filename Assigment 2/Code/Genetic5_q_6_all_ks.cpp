...
#define TARGET_HIT_HEURISTIC_BONUS 10 // bonus to give for each correct letter guessing
#define TARGET_HIT_HEURISTIC_BAD_GUESS_POINTS 0 //points to give for a bad guess
...
#define MAX_K_ITRS 1024 //half of them !
...
int main()
{
	...
	int best_k = 0;
	clock_t best_time = clock();

	for (int k = 1; k < (MAX_K_ITRS); k++) {
		...
		for (int i = 0; i < GA_MAXITER; i++) {
			...
			mate_selection(*population, *buffer,k);//mate using aging
		...
		if (!solved) {
			...
			cout << "Failed with K=" << k << endl;
		}
		cout << "===================================================" << endl;
		
	}
	cout << "Best K = "<<best_k<<" with time = "<< best_time << " clock ticks  = " << (((float)best_time) / CLOCKS_PER_SEC) << " seconds" << endl;
	return 0;
}
